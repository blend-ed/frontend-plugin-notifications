...
export async function getNotificationsList(appName, page, pageSize, trayOpened) {
  ...
  const {
    data
  } = await getAuthenticatedHttpClient().get(getNotificationsListApiUrl(), {
    params
  });
  return data;
}
export async function getNotificationCounts() {
  const {
    data
  } = await getAuthenticatedHttpClient().get(getNotificationsCountApiUrl());
  return data;
}
export async function markNotificationSeen(appName) {
  const {
    data
  } = await getAuthenticatedHttpClient().put(`${markNotificationsSeenApiUrl(appName)}`);
  return data;
}
export async function markAllNotificationRead(appName) {
  ...
  const {
    data
  } = await getAuthenticatedHttpClient().patch(markNotificationAsReadApiUrl(), params);
  return data;
}
