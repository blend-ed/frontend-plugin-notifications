...
import { getNotificationsListApiUrl, getNotificationsCountApiUrl, markNotificationsSeenApiUrl, markNotificationAsReadApiUrl } from './data/api';
...
export default async function mockNotificationsResponse() {
  let todaycount = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 8;
  let earlierCount = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 2;
  const axiosMock = new MockAdapter(getAuthenticatedHttpClient());
  const notifications = Factory.buildList('notification', todaycount, null, {
    createdDate: new Date().toISOString()
  }).concat(Factory.buildList('notification', earlierCount, null, {
    createdDate: '2023-06-01T00:46:11.979531Z'
  }));
