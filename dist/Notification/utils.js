...
import { QuestionAnswerOutline, PostOutline, Report, Verified, Newspaper, VideoCamera } from '@openedx/paragon/icons';
export const splitNotificationsByTime = notificationList => {
  let splittedData = [];
  if (notificationList.length > 0) {
    const currentTime = Date.now();
    const twentyFourHoursAgo = currentTime - 24 * 60 * 60 * 1000;
    splittedData = notificationList.reduce((result, notification) => {
      if (notification) {
...
export const getIconByType = type => {
  const iconMap = {
    ...
    live_session_starting_now: {
      icon: VideoCamera,
      class: 'text-success'
    },
    host_session_reminder: {
      icon: VideoCamera,
      class: 'text-primary-500'
    },
    host_session_starting_now: {
      icon: VideoCamera,
      class: 'text-success'
    }
  };
